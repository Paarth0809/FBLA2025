...
function displayOpt2EarthChapter9() {
    ...
    updateStoryText(chapter9Text);
    updateChoices([
        { text: "Smash through walls to find Appa faster", action: () => handleOpt2EarthChapter9Choice(1) },
        { text: "Protect Aang from Dai Li agents", action: () => handleOpt2EarthChapter9Choice(2) },
        { text: "Create underground tremors to confuse enemies", action: () => handleOpt2EarthChapter9Choice(3) },
        { text: "Rescue Jet from brainwashing", action: () => handleOpt2EarthChapter9Choice(4) }
    ]);
}
...
function handleOpt1EarthChapterChoice(choice) {
    ...
    setTimeout(() => {
        ...
        updateChoices([{ text: "Continue", action: startEarthChapter10 }]);
    }, 1500);
}
...
function handleOpt2EarthChapter8Choice(choice) {
    ...
    setTimeout(() => {
        updateStoryText(`<p>Appa is finally reunited with Aang as the Dai Li base collapses around you.</p>`);
         updateChoices([
                                   { text: "Continue", action: () => { startEarthChapter10(); playVideo('earthCutscene10.mp4'); } }
                               ]);
                           }, 300);
                       }
